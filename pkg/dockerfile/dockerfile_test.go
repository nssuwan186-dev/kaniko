...
func Test_SkipingUnusedStages(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			targets: []string{"base", ""},
			expectedSourceCodes: map[string][]string{
				"base":   {"FROM debian:12.13 as base"},
				"second": {"FROM debian:12.13 as base", "FROM scratch as second"},
				"":       {"FROM debian:12.13 as base", "FROM scratch as second", "FROM base as fourth", "FROM fourth"},
			},
			expectedTargetIndexBeforeSkip: map[string]int{
				"base":   0,
				"second": 1,
		...
		},
		{
			...
			expectedSourceCodes: map[string][]string{
				...
				"fizz":  {"FROM debian:12.13 as base", "FROM debian:12.13 as fizz"},
				"":      {"FROM alpine:3.11", "FROM debian:12.13 as base", "FROM debian:12.13 as fizz", "FROM alpine:3.11 as buzz", "FROM alpine:3.11 as final"},
			},
			expectedTargetIndexBeforeSkip: map[string]int{
				"final": 4,
				"buzz":  3,
